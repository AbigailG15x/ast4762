...
def fitgaussian(y, x=None, guess=None, lsargs={"maxfev":2000}, minout=None):
  ...
  '''
 def fitgaussian(y, x=None, guess=None, lsargs={"maxfev": 2000, "ftol": 1e-12, "xtol": 1e-12, "gtol": 1e-12}, minout=None):
    if x is None:
        x = np.indices(y.shape)
    else:
        if ((x.ndim == 1) and (x.shape != y.shape)) or ((x.ndim > 1) and (x.shape[1:] != y.shape)):
